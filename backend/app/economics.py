...
ASSUMPTIONS = [
    ...
    "The headline modelled gap is the earliest-required-date expected consequence minus the recommended expected consequence, on the same demand and supply. It is not observed savings and it is not a record of current practice.",
    "The current-practice proxy ranks firm orders, then due date, then margin and penalty. It ignores programme delay when it chooses, then the consequence still includes that delay, so its gap is an upper bound only.",
    "Low, base, and high rescale contractual penalty and delay cost per day to 60%, 100%, and 140% of the book, then solve again. Contribution margin is not scaled. The three results are assumption cases, not a confidence interval.",
